...
function ClientsTab({ exercises }) {
          ...
          <div style={{...css.modal(860)}}>
            <div style={{padding:"16px 22px",borderBottom:`1px solid ${C.border}`}}>
              ...
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                ...
                }} style={{...css.btnPrint,flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>🖨️ Spausdinti / PDF<button onClick={()=>{
  const c=view;
  const prog=view.program||{};
  const pn=view.program_name||"";
  const pl=progressList;
  const bv=calcBMI(c.weight,c.height);
  const bn=bv?parseFloat(bv.toFixed(1)):null;
  const bc=bn?bmiCat(bn):null;
  const nut=calcNut(c.weight,c.height,c.age,c.gender,ACTIVITY_LEVELS[c.activity_index??2]?.factor||1.55);
  const days=DAYS.filter(d=>(c.training_days||[]).includes(d));
  const today=new Date().toLocaleDateString("lt-LT");
  ...
  const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${pn} - ${c.name}</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .hdr{background:#0f1117;padding:20px 32px;display:flex;align-items:center;gap:16px}
    .logo{width:46px;height:46px;background:#c9a84c;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:800;color:#0f1117}
    .hdr-name{font-size:20px;font-weight:800;color:#c9a84c}
    .hdr-sub{font-size:10px;color:#888;letter-spacing:3px;text-transform:uppercase;margin-top:2px}
    .hdr-right{margin-left:auto;text-align:right;color:white}
    .sec{margin:20px 32px;border:1.5px solid #e0e0e8;border-radius:12px;overflow:hidden}
    .sh{background:#0f1117;color:white;padding:11px 20px;font-weight:700;font-size:12px;letter-spacing:2px;text-transform:uppercase;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    .ig{display:flex;flex-wrap:wrap;gap:10px;padding:16px 20px}
    .ib{background:#f5f5fa;border:1.5px solid #e0e0e8;border-radius:9px;padding:10px 16px;min-width:90px}
    .il{font-size:10px;color:#888;text-transform:uppercase;letter-spacing:1px;margin-bottom:3px}
    .iv{font-size:15px;font-weight:700}
    .er{display:flex;gap:14px;padding:14px 18px;border-top:1px solid #f0f0f5;align-items:flex-start;page-break-inside:avoid}
    .en{width:28px;height:28px;background:#f0f0f5;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;color:#555;flex-shrink:0;margin-top:4px}
    .ei{width:130px;height:100px;object-fit:cover;border-radius:9px;flex-shrink:0;border:1.5px solid #e8e8f0}
    .ep{width:130px;height:100px;background:#f0f0f5;border-radius:9px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:28px}
    .ename{font-size:15px;font-weight:700;margin-bottom:4px}
    .emus{font-size:12px;color:#4ea8a0;font-weight:600;margin-bottom:8px}
    .stats{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:6px}
    .sb{border-radius:7px;padding:5px 12px;display:inline-flex;flex-direction:column;align-items:center;border:1.5px solid}
    .sl{font-size:9px;color:#888;text-transform:uppercase;letter-spacing:1px}
    .sv{font-size:15px;font-weight:800}
    .edesc{font-size:11px;color:#777;font-style:italic;line-height:1.5}
    .foot{text-align:center;padding:20px;color:#aaa;font-size:11px;border-top:1px solid #eee;margin-top:16px}
    .print-btn{position:fixed;top:16px;right:16px;padding:12px 24px;background:#c9a84c;color:#000;border:none;border-radius:8px;font-family:inherit;font-weight:700;font-size:14px;cursor:pointer}
    @media print{.print-btn{display:none}}
  </style></head><body>
  <button class="print-btn" onclick="window.print()">🖨️ Spausdinti / Išsaugoti PDF</button>`;
  
  win.document.write(html);
  
  // Header
  win.document.write(`<div class="hdr"><div class="logo">M</div><div><div class="hdr-name">Coach Martynas</div><div class="hdr-sub">Sporto programa</div></div><div class="hdr-right"><div style="font-size:14px;font-weight:700">${pn||"Sporto programa"}</div><div style="font-size:11px;color:#888;margin-top:2px">Sukurta: ${today}</div></div></div>`);
  
  // Client info
  win.document.write(`<div class="sec"><div class="sh">👤 Kliento informacija</div><div class="ig">`);
  if(c.name) win.document.write(`<div class="ib"><div class="il">Vardas</div><div class="iv">${c.name}</div></div>`);
  if(c.age) win.document.write(`<div class="ib"><div class="il">Amžius</div><div class="iv">${c.age} m.</div></div>`);
  if(c.weight) win.document.write(`<div class="ib"><div class="il">Svoris</div><div class="iv">${c.weight} kg</div></div>`);
  if(c.height) win.document.write(`<div class="ib"><div class="il">Ūgis</div><div class="iv">${c.height} cm</div></div>`);
  if(bn) win.document.write(`<div class="ib" style="background:${bc.color}18;border-color:${bc.color}55"><div class="il">KMI</div><div class="iv" style="color:${bc.color}">${bn} — ${bc.label}</div></div>`);
  if(c.goal) win.document.write(`<div class="ib" style="background:#c9a84c18;border-color:#c9a84c55"><div class="il">Tikslas</div><div class="iv" style="color:#c9a84c">${c.goal}</div></div>`);
  if(c.level) win.document.write(`<div class="ib" style="background:#4ea8a018;border-color:#4ea8a055"><div class="il">Lygis</div><div class="iv" style="color:#4ea8a0">${c.level}</div></div>`);
  win.document.write(`</div>${c.notes?`<div style="padding:0 20px 14px;font-size:12px;color:#666;font-style:italic">📝 ${c.notes}</div>`:""}</div>`);
  
  // Nutrition
  if(nut){
    win.document.write(`<div class="sec"><div class="sh">🍽️ Mitybos rekomendacijos</div><div class="ig">`);
    win.document.write(`<div class="ib" style="background:#c9a84c18;border-color:#c9a84c55"><div class="il">TDEE</div><div class="iv" style="color:#c9a84c">${nut.tdee} kcal</div></div>`);
    win.document.write(`<div class="ib" style="background:#c0474a18;border-color:#c0474a55"><div class="il">🔻 Svorio metimas</div><div class="iv" style="color:#c0474a">${nut.lose} kcal</div></div>`);
    win.document.write(`<div class="ib" style="background:#f8717118;border-color:#f8717155"><div class="il">Baltymai (met.)</div><div class="iv" style="color:#f87171">${nut.protLose} g</div></div>`);
    win.document.write(`<div class="ib" style="background:#4ade8018;border-color:#4ade8055"><div class="il">🔺 Raumenų auginimas</div><div class="iv" style="color:#4ade80">${nut.gain} kcal</div></div>`);
    win.document.write(`<div class="ib" style="background:#4ea8a018;border-color:#4ea8a055"><div class="il">Baltymai (aug.)</div><div class="iv" style="color:#4ea8a0">${nut.protGain} g</div></div>`);
    win.document.write(`</div></div>`);
  }
  ...
  days.forEach(day=>{
    const exs=(prog[day]||[]);
    win.document.write(`<div class="sec"><div class="sh">${day} — ${exs.length} pratimas(-ai)</div>`);
    if(!exs.length) win.document.write(`<div style="padding:14px 20px;color:#aaa;font-size:13px">Pratimų nėra</div>`);
    exs.forEach((ex,i)=>{
      const imgs=(ex.imgs||[]).filter(Boolean);
      win.document.write(`<div class="er"><div class="en">${i+1}</div>`);
      if(imgs[0]) win.document.write(`<img src="${imgs[0]}" class="ei"/>`);
      else win.document.write(`<div class="ep">📷</div>`);
      if(imgs[1]) win.document.write(`<img src="${imgs[1]}" style="width:90px;height:70px;object-fit:cover;border-radius:8px;border:1.5px solid #e8e8f0;margin-top:4px;flex-shrink:0"/>`);
      win.document.write(`<div style="flex:1"><div class="ename">${ex.name}</div><div class="emus">${ex.muscle||""} · ${ex.equipment||""}</div><div class="stats">`);
      if(ex.customSets) win.document.write(`<span class="sb" style="background:#c9a84c18;border-color:#c9a84c55"><span class="sl">Serijos</span><span class="sv" style="color:#c9a84c">${ex.customSets}</span></span>`);
      if(ex.customReps) win.document.write(`<span class="sb" style="background:#f5f5fa;border-color:#ddd"><span class="sl">Kartojimai</span><span class="sv" style="color:#333">${ex.customReps}</span></span>`);
      if(ex.customWeight) win.document.write(`<span class="sb" style="background:#4ea8a018;border-color:#4ea8a055"><span class="sl">Svoris</span><span class="sv" style="color:#4ea8a0">${ex.customWeight}</span></span>`);
      if(ex.customRest) win.document.write(`<span class="sb" style="background:#a78bfa18;border-color:#a78bfa55"><span class="sl">Poilsis</span><span class="sv" style="color:#a78bfa">${ex.customRest}</span></span>`);
      win.document.write(`</div>${ex.description?`<div class="edesc">${ex.description}</div>`:""}</div></div>`);
    });
    win.document.write(`</div>`);
  });
  ...
  // Progress
  if(pl&&pl.length>0){
    win.document.write(`<div class="sec"><div class="sh">📈 Pažangos istorija</div><div style="padding:14px 20px"><table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr style="background:#f5f5fa"><th style="padding:8px 12px;text-align:left;font-size:11px;color:#888;text-transform:uppercase;border-bottom:1.5px solid #e0e0e8">Data</th><th style="padding:8px 12px;text-align:left;font-size:11px;color:#888;text-transform:uppercase;border-bottom:1.5px solid #e0e0e8">Svoris</th><th style="padding:8px 12px;text-align:left;font-size:11px;color:#888;text-transform:uppercase;border-bottom:1.5px solid #e0e0e8">Krūtinė</th><th style="padding:8px 12px;text-align:left;font-size:11px;color:#888;text-transform:uppercase;border-bottom:1.5px solid #e0e0e8">Juosmuo</th><th style="padding:8px 12px;text-align:left;font-size:11px;color:#888;text-transform:uppercase;border-bottom:1.5px solid #e0e0e8">Klubai</th><th style="padding:8px 12px;text-align:left;font-size:11px;color:#888;text-transform:uppercase;border-bottom:1.5px solid #e0e0e8">Pastabos</th></tr></thead><tbody>`);
    pl.forEach((p,i)=>win.document.write(`<tr style="background:${i%2?"#fafafa":"white"}"><td style="padding:8px 12px;border-bottom:1px solid #f0f0f5">${new Date(p.date).toLocaleDateString("lt-LT")}</td><td style="padding:8px 12px;border-bottom:1px solid #f0f0f5;font-weight:700;color:#c9a84c">${p.weight?p.weight+" kg":"—"}</td><td style="padding:8px 12px;border-bottom:1px solid #f0f0f5">${p.chest?p.chest+" cm":"—"}</td><td style="padding:8px 12px;border-bottom:1px solid #f0f0f5">${p.waist?p.waist+" cm":"—"}</td><td style="padding:8px 12px;border-bottom:1px solid #f0f0f5">${p.hips?p.hips+" cm":"—"}</td><td style="padding:8px 12px;border-bottom:1px solid #f0f0f5;font-style:italic;color:#666">${p.notes||"—"}</td></tr>`));
    win.document.write(`</tbody></table></div></div>`);
  }
  
  win.document.write(`<div class="foot">© Coach Martynas · ${today}</div></body></html>`);
  win.document.close();
